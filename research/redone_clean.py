...
# Get every non-anomalous row
non_anomalous_values = X__train[
    transformed_dataset["Anomaly"] == 1
][columns]

anomalous_values = X__train[
    transformed_dataset["Anomaly"] == -1
][columns]
print(len(non_anomalous_values))
print(len(anomalous_values))
...
pd.DataFrame(
    X__test,
    columns=non_anomalous_values.columns
).to_excel(
    "testing_data.xlsx",
    index=False
)