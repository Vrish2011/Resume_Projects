    ...
    for row in dict_diet:
        for key, value in row.items():
            ...
            if not key in numerical_columns and not key in categorical_columns:
                ...
                elif key not in bool_columns and not is_numerical:
...
plt.savefig(
    "Cluster_analysis_hierarchel.png",
    dpi=300,
    bbox_inches="tight"
)
...
official_df.to_excel(
    "clustered_dataset.xlsx"
)
...
cluster_means_kmeans.to_excel(
    "Kmeans_analyzed_numeric.xlsx"
)

cluster_means_agg.to_excel(
    "Agg_analyzed_numeric.xlsx"
)


cluster_means_kmeans_cat = (
    transformed_dataset
    .groupby("Cluster_kmeans")[analysis_columns]
    .mean()
)

cluster_means_agg_cat = (
    transformed_dataset
    .groupby("Cluster_agg")[analysis_columns]
    .mean()
)


cluster_means_kmeans_cat.to_excel(
    "Kmeans_analyzed_categorical.xlsx"
)

cluster_means_agg_cat.to_excel(
    "Agg_analyzed_cat.xlsx"
)